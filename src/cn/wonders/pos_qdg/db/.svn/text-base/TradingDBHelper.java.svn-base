 ...
 */
public class TradingDBHelper extends OrmLiteSqliteOpenHelper {
	
	private static final String TABLE_NAME = "pos_liu_shui.db";
	
	private Map<String, Dao> daos = new HashMap<String, Dao>();
	
	private static TradingDBHelper instance;
	
	
	private TradingDBHelper(Context context)
	{
	 ...
	 * @return
	 */
	public static synchronized TradingDBHelper getHelper(Context context)
	{
		...
		if (instance == null)
		{
			...
			{
				if (instance == null)
					instance = new TradingDBHelper(context);
			}
		}
...
//	}
	...
	public void onCreate(SQLiteDatabase database,
		...
		try
		{
			TableUtils.createTable(connectionSource, TradingBean.class);
	...
	public void onUpgrade(SQLiteDatabase database,
		...
		if(oldVersion<7 ){
			try {
				...
				getDao(TradingBean.class).executeRaw("ALTER TABLE `tb_tran_list` ADD COLUMN is_abnormal_up Integer DEFAULT 0;");
